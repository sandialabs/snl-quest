...
class quest_workflow(QWidget):
    def __init__(self, parent=None):
        ...
        self.inputs_management_label = QLabel("Manage all data node names, variables, and values.")
        self.inputs_management_label.setWordWrap(True)
        self.inputs_management_table = QTableWidget()
        self.inputs_management_table.setColumnCount(5)
        self.inputs_management_table.setHorizontalHeaderLabels(["Node Name", "Variable Name", "Value", "Is Path?", "Is From Master?"])
        self.inputs_management_table.verticalHeader().setVisible(False)
        self.inputs_management_table.setSelectionBehavior(QAbstractItemView.SelectItems)
        self.inputs_management_table.setSelectionMode(QAbstractItemView.SingleSelection)
        self.inputs_management_table.setEditTriggers(
            QAbstractItemView.DoubleClicked
            | QAbstractItemView.EditKeyPressed
            | QAbstractItemView.AnyKeyPressed
        )
        self.inputs_management_table.setAlternatingRowColors(True)
        self.inputs_management_table.setStyleSheet(
            "QTableWidget {"
            "border: 1px solid #d9e2ec;"
        ...
        self.inputs_management_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.inputs_management_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)
        self.inputs_management_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.Stretch)
        self.inputs_management_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeToContents)
        self.inputs_management_table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeToContents)
        self.inputs_management_table.itemChanged.connect(self._on_inputs_management_item_changed)
        ...
        self.inputs_management_layout.addWidget(self.inputs_management_table)
        self.inputs_management_layout.addWidget(self.inputs_management_save_button)
        self.inputs_management_layout.addWidget(self.inputs_management_load_button)
    ...
    def _refresh_inputs_management_table(self):
        if not hasattr(self, 'inputs_management_table'):
            return

        self._inputs_management_refresh_in_progress = True
        try:
            self.inputs_management_table.blockSignals(True)
            is_master_flow = (self.get_flow_type() == "master-flow")
            data_nodes = sorted(
                self._data_nodes(),
                key=lambda node: str(node.name() or '').lower()
            )
            self.inputs_management_table.setRowCount(len(data_nodes))
            for row, node in enumerate(data_nodes):
                values = [
                    str(node.name() or ''),
                    str(getattr(node, 'node_input_variable', '') or ''),
                    str(getattr(node, 'node_input_value', '') or ''),
                ]
                for column, value in enumerate(values):
                    item = QTableWidgetItem(value)
                    item.setData(Qt.UserRole, node.id)
                    self.inputs_management_table.setItem(row, column, item)
                path_combo = QComboBox()
                path_combo.addItems(["False", "True"])
                path_combo.setCurrentText("True" if bool(getattr(node, 'node_is_path', False)) else "False")
                path_combo.currentTextChanged.connect(
                    lambda text, node_id=node.id: self._on_inputs_management_path_changed(node_id, text)
                )
                self.inputs_management_table.setCellWidget(row, 3, path_combo)
                from_master_combo = QComboBox()
                from_master_combo.addItems(["False", "True"])
                from_master_combo.setCurrentText("True" if bool(getattr(node, 'node_is_from_master', False)) else "False")
                from_master_combo.setEnabled(not is_master_flow)
                from_master_combo.currentTextChanged.connect(
                    lambda text, node_id=node.id: self._on_inputs_management_from_master_changed(node_id, text)
                )
                self.inputs_management_table.setCellWidget(row, 4, from_master_combo)
            if len(data_nodes) == 0:
                self.inputs_management_table.setRowCount(0)
        finally:
            self.inputs_management_table.blockSignals(False)
            self._inputs_management_refresh_in_progress = False

    def _on_inputs_management_path_changed(self, node_id, value):
        if self._inputs_management_refresh_in_progress:
            return
    ...
    def _on_inputs_management_from_master_changed(self, node_id, value):
        if self._inputs_management_refresh_in_progress:
            return
    ...
    def _on_inputs_management_item_changed(self, item):
        if self._inputs_management_refresh_in_progress or item is None:
            return
    ...
    def _save_inputs_management_yaml(self):
        records = self._inputs_management_records()
    ...
    def _load_inputs_management_yaml(self):
        ...
        if not isinstance(records, list):
            QMessageBox.warning(
                self,
                "Invalid YAML",
                "The selected YAML file must contain an 'inputs' list."
            )
            return

        nodes_by_name = {str(node.name() or "").strip(): node for node in self._data_nodes()}
        matches = []
        unmatched = []
        ...
        if not matches:
            detail = ""
            if unmatched:
                detail = "\n\nNo matching data nodes were found.\nUnmatched node names:\n- " + "\n- ".join(unmatched)
            QMessageBox.information(
                self,
                "No Matching Inputs",
                f"No matching data nodes were found in:\n{path}{detail}"
            )
            return
        ...
        for node, entry in matches:
            ...
            if "variable_name" in entry:
                fields.append(f"variable -> {entry.get('variable_name', '')}")
            if "value" in entry:
                fields.append(f"value -> {entry.get('value', '')}")
            if "is_path" in entry:
                fields.append(f"is_path -> {entry.get('is_path')}")
            if "is_from_master" in entry:
                if self.get_flow_type() == "master-flow":
                    fields.append("is_from_master -> False (master flow)")
                else:
                    fields.append(f"is_from_master -> {entry.get('is_from_master')}")
            summary_lines.append(f"- {node.name()}: " + ", ".join(fields if fields else ["no editable fields found"]))

        summary_text = "\n".join(summary_lines)
        if unmatched:
            summary_text += "\n\nUnmatched YAML node names:\n- " + "\n- ".join(unmatched)

        reply = QMessageBox.question(
            self,
            "Load Inputs from YAML",
            f"The following node data will be populated from:\n{path}\n\n{summary_text}\n\nProceed?",
        ...
        try:
            for node, entry in matches:
                self._apply_inputs_yaml_record_to_node(node, entry)
            self._refresh_inputs_management_table()
            self._sync_parent_proxy_wrapper_from_current_graph()
            self.on_node_selected()
        except Exception as e:
