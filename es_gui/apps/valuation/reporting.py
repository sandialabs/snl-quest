...
from es_gui.resources.widgets.common import TWO_ABC_WIDTH, THREE_ABC_WIDTH, MyPopup
...
class Report(Screen):
    ...
    def __init__(self, chart_data, report_attributes, market=None, **kwargs):
        ...
        for opt in self.chart_types.items():
            ...
            screen = ReportScreen(type=opt[1], chart_data=self.chart_data, market=self.market, name=opt[1])
    ...
    def add_report(self, chart_type, *args):
        ...
        if not sm.has_screen(chart_type):
            screen = ReportScreen(type=chart_type, chart_data=self.chart_data, market=self.market, name=chart_type)
            sm.add_widget(screen)
...
class ReportScreen(Screen):
    ...
    def generate_revenue_bar_chart(self, *args):
        ...
        # select chart colors
        if len(self.chart_data) > len(self.palette):
            colors = self.palette
        else:
            colors = sample(self.palette, len(self.chart_data))
    ...
    def generate_revenue_multisetbar_chart(self, *args):
        ...
        # select chart colors
        if n_rev_cats > len(self.palette):
            colors = self.palette
        else:
            colors = sample(self.palette, n_rev_cats)
    ...
    def generate_activity_donut_chart(self, market=None, *args):
        ...
        # select chart colors
        if n_activity_cats > len(self.palette):
            colors = self.palette
        else:
            colors = sample(self.palette, n_activity_cats)
