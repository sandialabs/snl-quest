...
def perform_tech_selection(selections, target_cost_kWh, target_cost_kW):
    ...
    # Read databases
    # tech_data = pd.read_excel(os.path.join('es_gui', 'apps', 'tech_selection', 'ES_Tech_Database.xlsx'), index_col='Storage technology')
    ...
    tech_data.rename(columns={'Feas. score for residential': 'Score for BTM: residential',
                                      ...
                                      'Feas. score for transmission': 'Score for Transmission/central'},
                             inplace=True)
    ...
    feas_response_time = list(set(fFeasibility.isFeasibleTech_ResponseTime(tech_data, app_data.loc[application, 'Minimum required response time'])))
    feas_electric_output = list(set(fFeasibility.isFeasibleTech_ElectricOutput(tech_data, app_data.loc[application, 'Requires electric output'])))
    ...
    all_feasibility['Application requirements'] = [value in feas_duration and value in feas_response_time and value in feas_electric_output for value in all_feasibility.index]
