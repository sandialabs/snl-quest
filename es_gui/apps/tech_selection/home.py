...
class TechSelectionHomeScreen(Screen):
    """The home screen for doing energy storage technology selection analysis."""
    ...
    def open_help_carousel(self, *args):
        ...
        slide_01_text = "QuESt Technology Selection is an application with tools for identifying feasible storage technlogies for a given project."
        slide_02_text = "The technology selection wizard identifies which energy storage technologies satisfy the minimum application requirements (such as discharge duration and response time) and rank them. The user is required to indicate the grid location where the storage system will be deployed, which set default values for the other types of inputs."
        slide_03_text = "Upon completion of the wizard, you will be taken to the first results screen, which indicates whether each energy storage technology is a feasible option for the desired project."
        slide_04_text = "The next results screen displays a rank of the feasible technologies based on four factors. The user can modify the weights assigned to each one of these factors, as well as the desired target cost (the latter affects the Cost Score). These adjustments will be reflected in the new Total Score for each technology."
