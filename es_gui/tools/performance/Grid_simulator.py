...
if os.path.exists('C:/EnergyPlusV9-4-0'):
    sys.path.insert(0,'C:/EnergyPlusV9-4-0')
else:
    print('Either Energyplus is not downloaded or it is not in the correct directory')
...
#        pass
    ...
    def _begin_new_environment_func(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_begin_new_environment'.
        """
        pass
    
    def _after_new_environment_warmup_complete(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_after_new_environment_warmup_complete'.
        """
        pass
    
    def _begin_zone_timestep_before_init_heat_balance(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_begin_zone_timestep_before_init_heat_balance'.
        """
        pass
    
    def _begin_zone_timestep_after_init_heat_balance(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_begin_zone_timestep_after_init_heat_balance'
        """
        pass
    
    def _begin_system_timestep_before_predictor(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_begin_system_timestep_before_predictor'
        """
        sys.stdout.flush()
        ...
        if self.one_time: 
            if self.exchange.api_data_fully_ready(state):              
                try:
                    self.variablesF['Handle'] = [self.exchange.get_variable_handle(state,vtype,key) for vtype,key in self.variablesF[['Variable Type','Variable Key']].values]
                    self.actuatorsF['Handle'] = [self.exchange.get_actuator_handle(state,atype,actype,akey) for atype,actype,akey in self.actuatorsF[['Component Type','Control Type','Actuator Key']].values]
        ...
        if self.exchange.warmup_flag(state):
            ...
            try:
                self.exchange.set_actuator_value(state,int(self.actuatorsF['Handle'].loc[(self.actuatorsF[['Control Type','Actuator Key']] == ['Heating Setpoint', 'Zone One']).all(axis=1)].values), self.h_setpoint)
                self.exchange.set_actuator_value(state,int(self.actuatorsF['Handle'].loc[(self.actuatorsF[['Control Type','Actuator Key']] == ['Cooling Setpoint', 'Zone One']).all(axis=1)].values), self.c_setpoint)
    ...
    def _begin_zone_timestep_before_set_current_weather(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_begin_zone_timestep_before_set_current_weather'
        """
        pass
    
    def _after_predictor_before_hvac_managers(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_after_predictor_before_hvac_managers'
        """
        pass
    
    def _after_predictor_after_hvac_managers(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_after_predictor_after_hvac_managers'
        """
        pass
    
    def _inside_system_iteration_loop(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_inside_system_iteration_loop'
        """
        environ = self.exchange.current_environment_num(state)
        if self.exchange.warmup_flag(state):
            pass
    ...
    def _end_zone_timestep_before_zone_reporting(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_end_zone_timestep_before_zone_reporting'
        """
        pass
    
    def _end_zone_timestep_after_zone_reporting(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_end_zone_timestep_after_zone_reporting'
        """
        sys.stdout.flush() 
...
#                mainthread(self.sm.get_screen('progress').increment_progress)
    ...
    def _end_system_timestep_before_hvac_reporting(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_end_system_timestep_before_hvac_reporting'
        """
        pass
    
    def _end_system_timestep_after_hvac_reporting(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_end_system_timestep_after_hvac_reporting'
        """
        environ = self.exchange.current_environment_num(state)
        if self.exchange.warmup_flag(state):
            pass
    ...
    def _end_zone_sizing(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_end_zone_sizing'
        """
        pass
    
    def _end_system_sizing(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_end_system_sizing'
        """
        pass
    
    def _after_component_get_input(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_after_component_get_input'
        """
        pass
    
    def _unitary_system_sizing(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_unitary_system_sizing'
        """
        pass
    
    def _register_external_hvac_manager(self, state: c_void_p) -> None:
        """
            Function to pass to Eplus callback 'callback_register_external_hvac_manager'
        """
        pass
    ...
    def _default_callback_funcs(self):
        ...
        def time_step_handler_first(self,state):
            """
                Function to callback at the beginning of each timestep in E+. Generally
                for setting variables. The API requires that there are no inputs or outputs
                to this function. Should be called at 'begin timestep before predictor'.
            """
            ...
            if self.exchange.warmup_flag(state):
                ...
                try:
                    self.exchange.set_actuator_value(state,self.actuatorsF['Handle'].loc[(self.actuatorsF[['Control Type','Actuator Key']] == ['Heating Setpoint', 'Zone One']).all(axis=1)], self.h_setpoint)
                    self.exchange.set_actuator_value(state,self.actuatorsF['Handle'].loc[(self.actuatorsF[['Control Type','Actuator Key']] == ['Cooling Setpoint', 'Zone One']).all(axis=1)], self.c_setpoint)
        ...
        def time_step_handler_system(self,state):
            """
                Function to callback at in the hvac iteration loop. Use to link HVAC load
                with battery.
            """
        ...
        def time_step_handler_system_after_HVAC(self,state):
            """
                After HVAC reporting at the system timestep.
            """
        ...
        def time_step_handler_last(self,state): 
            """
                Function to callback at the end of each timestep. For collecting variable
                values. Should be called at 'end timestep after zone reporting'.
            """
    ...
    def simulate_battery(self):
        ...
        if not self.charge_discharge.empty:
            ...
            if not self.battery.p_d == 0 and self.hvac_flag:
                self.battery.p_d += self.exchange.get_variable_value(self.state,
                                int(self.variablesF['Handle'].loc[(self.variablesF[['Variable Type','Variable Key']] == ['Facility Total HVAC Electricity Demand Rate','Whole Building']).all(axis=1)].values))/1e6
                                                                     
        elif not self.load.empty:
            self.battery.p_c = 0
    ...
    def append_battery_data(self):
        ...
        temp_lst = [#self.exchange.month(self.state), self.exchange.day_of_month(self.state), self.exchange.hour(self.state),
                    self.battery.soc_end, self.battery.p_c, self.battery.p_d, self.battery.heat_loss]
        self.bdata_lst.append(temp_lst)
        
#    def increment_progress(self, *args):
#        """
#            Increment Quest progress bar.
#        """
#        
#        self.progress_bar = self.progress 
    ...
    def get_results(self):
        ...
        self.bdataF = pd.DataFrame(self.bdata_lst, columns = [#'Month','Day','Hour',
                    'SOC','Charge Power','Discharge Power','Heat Loss'])
