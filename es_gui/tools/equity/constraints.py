...
def ineq_gen_matching_rule(m):
    ...
    def _gen_matching_rule(battery,i):
        if mp.flexible_dispatch:
            dsp = mp.plant_dispatch[i]*mp.meets[i]
        if mp.fixed_dispatch:
            dsp = mp.filtered_plant_dispatch[i]
        return -mp.pe_d[i] - mp.pe_c[i] + mp.pv_capacity*mp.pv_profile[i] >= dsp
...
def ineq_meets_rule(m):
    ...
    def _meets_rule(battery):
        ...
        return mp.replacement_fraction*sum(mp.plant_dispatch) <= sum([mp.plant_dispatch[i]*mp.meets[i] for i in operating])
