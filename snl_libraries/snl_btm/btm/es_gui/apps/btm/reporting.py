...
from btm.es_gui.proving_grounds.charts import BarChart, StackedBarChart, MultisetBarChart, PieChart, DonutChart, format_dollar_string
from btm.es_gui.resources.widgets.common import TWO_ABC_WIDTH, THREE_ABC_WIDTH, MyPopup, TileButton, PALETTE, rgba_to_fraction, ReportScreen, WizardReportInterface, ReportChartToggle
...
class BtmCostSavingsGenerateReportMenu(ModalView):
    ...
    def save_figure(self, screen, *args):
        ...
        chart_images_dir = os.path.join(dirname, 'results', 'btm_cost_savings', 'report', self.report_id, 'images')
        ...
        self.graphics_locations[screen.name] = os.path.join(chart_images_dir, 'chart_{n}.png'.format(n=screen.name))
    ...
    def generate_report_from_template(self):
        ...
        template_dir = os.path.join(dirname, 'es_gui', 'resources', 'report_templates')
        output_dir_name = self.report_id

        output_dir = os.path.join(dirname, 'results', 'btm_cost_savings', 'report', output_dir_name)
