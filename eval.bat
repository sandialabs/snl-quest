...
set "VENV_PATH=%APP_ENVS_PATH%\env_eval"

REM Install Valuation from the QuESt_Valuation branch without requiring Git on the target machine.
set "EVAL_BRANCH=QuESt_Valuation"
set "EVAL_PACKAGE_URL=https://github.com/sandialabs/snl-quest/archive/refs/heads/%EVAL_BRANCH%.zip"

REM Legacy Valuation requires exactly Python 3.9.13 on Windows.
...
if exist "%VENV_PATH%" (
    if not exist "%VENV_PATH%\Scripts\activate.bat" (
        ...
        if errorlevel 1 (
            echo Existing Valuation environment is not using Python %PYTHON_VERSION%. Recreating it...
            call :reset_venv
        )
    )
)
...
python -m pip install --upgrade --force-reinstall --trusted-host files.pythonhosted.org --trusted-host pypi.org --trusted-host pypi.python.org --trusted-host github.com --trusted-host codeload.github.com "%EVAL_PACKAGE_URL%"
if errorlevel 1 (
    echo Failed to install QuESt Valuation from branch "%EVAL_BRANCH%".
    exit /b 1
)
...
if errorlevel 1 (
    echo QuESt Valuation installed, but its Python dependencies could not be imported.
    exit /b 1
)
