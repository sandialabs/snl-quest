...
### Install Miniconda
Before proceeding, install Miniconda by following the instructions at [Miniconda Installation Guide](https://docs.anaconda.com/miniconda/). If you have a previous installation of Miniconda or Anaconda, you may jump to step 4.

4. Open the Miniconda Command Prompt.

5. Create a new conda environment with Python 3.9.13 by running:
   ```bash
...
7. Navigate to the extracted folder:
   ```bash
   cd path/to/snl-QuESt-2.0.b


### For Windows Users:
8. Run the `win_entry.bat` file from inside the environment (in the Miniconda Command Prompt):
   ```bash
   .\win_entry.bat
    ```

   - This will run the batch file, check for and install any missing dependencies (Git, python packages), create a virtual env, and launch the QuESt application.

   After this initial run users can double click on the win_entry.bat to launch QuESt.



### For Unix Users (Linux and macOS):

8. Make the script executable by running (In the mininconda command prompt):
   ```bash
   chmod +x unix_entry.sh
    ```

9. Double-click the file called unix_entry.sh or run it from the terminal (From within the activated env, some systems may require users to always run QuESt from the terminal):
   ```bash
    ./unix_entry.sh
    ```
...
- In future runs of QuESt users may just open the miniconda terminal and run the following command to launch QuESt:
   ```bash
   ./unix_entry.sh
    ```
